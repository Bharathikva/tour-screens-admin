  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen space-y-8 lg:space-y-0 lg:space-x-8">
      ...
      <div className="w-full lg:w-1/4 bg-white border p-6 rounded-xl shadow-lg">
        ...
        <div
          className={`relative w-full p-4 border-2 rounded-md transition-all duration-300 ${dragging ? "border-dashed border-blue-500 bg-blue-100" : "border-gray-300"
            }`}
          onDrop={handleDrop}
        ...
        <div className="overflow-y-auto max-h-[400px] space-y-4 mt-4">
          {getUploadedImages().map((image, index) => (
            <div
              key={index}
              className="w-full flex justify-center items-center relative transition-all duration-300 "
      ...
      <div className="flex-1 bg-white border p-6 rounded-xl shadow-xl">
        {selectedImage ? (
          <>
            ...
            <div className="bg-white w-full max-w-2xl h-auto overflow-auto relative">
              ...
              <div className="relative w-full h-full" onClick={handleImageClick}>
                ...
                {annotations.map((annotation, index) => (
                  ...
                  >
                    <div
                      className="relative text-black text-sm px-2 py-1 rounded shadow-lg"
                      style={{
                        backgroundColor: 'rgba(255, 255, 0, 0.5)', // Semi-transparent yellow
                        backdropFilter: 'blur(2px)', // Optional: Adds a subtle blur effect
                      }}
                    >
                      {annotation.content}
                      {/* Arrow below the content */}
                      <div
                        className="absolute w-0 h-0 border-l-8 border-r-8 border-t-8"
                        style={{
                          borderTopColor: 'rgba(255, 255, 0, 0.5)', // Matches the transparent content background
